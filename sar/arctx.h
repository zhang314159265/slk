...
/*
 * A helper verifies that the archive file can fully resolve all the symbols in
 * 'target_syms' assuming 'predefined_syms' are predefined.
 *
 * target_syms will be mutated.
 *
 * This helper can be used to decide the minimal list of object files needed
 * to resolve all symbols in `target_syms`. This is a bit more complex then what
 * slk does. In slk, we just need to gather the list of defined symbols and use
 * that to resolve symbol references.
 */
static void arctx_resolve_symbols(struct arctx* ctx, struct vec* target_syms, struct vec* predefined_syms) {
  ...
  while (target_syms->len > 0) {
	...
	if (getenv("DUMP_NEEDED_OBJ")) {
  	const char* path = "/tmp/needed_obj";
  	printf("Write %d needed object files to %s\n", added_file.size, path);
  	FILE* fp = fopen(path, "w");
